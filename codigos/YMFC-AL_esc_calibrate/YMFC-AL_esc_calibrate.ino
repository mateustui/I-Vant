...
//Main program loop
void loop(){
  while(zero_timer + 10000 > micros());                                                  //Start the pulse after 10000 micro seconds.
  ...
  if(data == 'a'){
    ...
    if(cal_int != 2000){
      ...
      //0.0000026646 = 0.00015267  * (3.142(PI) / 180degr) The Arduino sin function is in radians
      angle_pitch -= angle_roll * sin(gyro_yaw * 0.0000026646 );                         //If the IMU has yawed transfer the roll angle to the pitch angel.
      angle_roll += angle_pitch * sin(gyro_yaw * 0.0000026646 );                         //If the IMU has yawed transfer the pitch angle to the roll angel.
